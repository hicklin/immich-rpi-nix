...
7. Setup the RPi server
   1. Setup the RPi following [setup step 2](../README.md#2-raspberry-pi-setup).
   2. Setup immich following [setup step 3](../README.md#3-immich-setup) up to and including step 6.
      > [!IMPORTANT]
      > Use the same `DB_PASSWORD` used in step 4 above.
   3. Fix permissions and ownership - Issue with different postgres versions. generate and use backup instead.
      ```bash
      sudo chown -R immich:users /mnt/immich_drive/immich_data
      sudo chown -R postgres:users /mnt/immich_drive/postgres
      ```
   4. Start immich
      ```bash
      sudo systemctl reset-failed postgresql
      sudo systemctl start postgresql
      sudo systemctl start immich-server
      ```
   5. Access the server and confirm that all the photos are there.
   6. Configure backups following [setup step 4](../README.md#4-configure-backups)
   7. Setup remote access following [setup step 5](../README.md#5-remote-access)
